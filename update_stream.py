...
import jsonimport reimport subprocessimport sysimport requestsfrom datetime import datetimefrom pathlib import Pathfrom typing import Dict, List, Optional

CONFIG_FILE = "config.json"COOKIE_FILE = "cookies.txt"

CHROME_UA = ("Mozilla/5.0 (Windows NT 10.0; Win64; x64) ""AppleWebKit/537.36 (KHTML, like Gecko) ""Chrome/148.0.0.0 Safari/537.36")

def load_config() -> Dict:with open(CONFIG_FILE, "r", encoding="utf-8") as f:config = json.load(f)
...
def safe_filename(name: str) -> str:replacements = {"ç": "c", "Ç": "C","ğ": "g", "Ğ": "G","ı": "i", "İ": "I","ö": "o", "Ö": "O","ş": "s", "Ş": "S","ü": "u", "Ü": "U",}for old, new in replacements.items():name = name.replace(old, new)

name = re.sub(r"[^A-Za-z0-9._-]+", "_", name).strip("_")
return f"{name or 'channel'}.m3u"

def is_direct_m3u8(url: str) -> bool:clean = url.lower().split("?", 1)[0]return clean.endswith(".m3u8")

def normalize_channel_name(name: str) -> str:"""Kanal adını karşılaştırma için normalize eder.
...
def is_eurostar_name(name: str) -> bool:lower = normalize_channel_name(name)return "euro star" in lower or "eurostar" in lower or "star avrupa" in lower

def is_show_turk_name(name: str) -> bool:lower = normalize_channel_name(name)return "show" in lower and ("türk" in lower or "turk" in lower)

def load_netscape_cookies(cookie_file: str = COOKIE_FILE) -> Dict[str, str]:"""Netscape format cookies.txt dosyasını requests cookies dict formatına çevirir."""cookies: Dict[str, str] = {}path = Path(cookie_file)
...
def get_eurostar_token() -> Optional[str]:"""EuroStar 1080p - HTML'den token çek."""headers = {"User-Agent": CHROME_UA,"Accept": "text/html,application/xhtml+xml",}
...
def get_show_turk_token() -> Optional[str]:"""Show Türk - otomatik token alıcı."""url = "https://www.showturk.com.tr/canli-yayin"pattern = r"playlist.m3u8?e=(\d+)&st=([^"\s&]+)"
...
def _run_ytdlp(cmd: List[str], label: str, timeout: int = 140) -> List[str]:"""yt-dlp çalıştır, çıktıyı temizle ve hata varsa kısa göster."""try:result = subprocess.run(cmd,capture_output=True,text=True,timeout=timeout,)except Exception as e:print(f"   ❌ yt-dlp çalıştırılamadı ({label}): {e}")return []
...
def _pick_stream_from_lines(lines: List[str]) -> Optional[str]:"""yt-dlp -g çıktısından oynatılabilir URL seç."""if not lines:return None
...
def get_youtube_stream_url(youtube_url: str, quality: str) -> Optional[str]:"""YouTube canlı yayın URL'sini al.
...
def get_stream_url(channel: Dict, quality: str) -> Optional[str]:"""Kanal tipine göre stream URL'sini al."""channel_name = channel.get("name", "")
...
def create_extinf(channel: Dict, stream_url: str) -> str:name = channel["name"]logo = channel.get("logo", "")group = channel.get("group", "Genel")tvg_id = channel.get("tvg_id", safe_filename(name).replace(".m3u", "").lower())
...
def write_main_playlist(entries: List[str], output_folder: Path, output_playlist: str) -> Path:"""Ana playlisti direkt stream URL'leriyle yazar."""output_folder.mkdir(parents=True, exist_ok=True)
...
def main() -> int:print("=" * 60)print("🎬 TV Kanalları M3U Güncelleyici (Token + Cookie Desteği)")print("=" * 60)print(f"🕐 Başlangıç: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")print("=" * 60)
